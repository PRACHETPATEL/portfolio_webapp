window.addEventListener('load',(event)=>{
    ...
    contactform.addEventListener('submit',async (e)=>{
        ...
        const response=await axios.post(api+'/api/contact',{name:name.value, email:email.value, message:message.value});
        name.value="";
        email.value="";
        message.value="";
        if(response.data.status===200){
            setTimeout(()=>{
                ...
                document.getElementById("tooltip").innerText=response.data.message;
                setTimeout(()=>{
